...
const amount=s=>Math.round(Number(s.replace(/[^0-9.]/g,''))*100);
(async()=>{
 ...
 try{
  for(const[name,width,height]of[['desktop',1440,1000],['mobile',390,844]]){
   ...
   const detail=page.locator('.pd-detail-info'),values=await detail.locator('.pd-value-breakdown dd').allTextContents();
   ...
   const fresh=await page.locator('.pd-value-breakdown dd').allTextContents();assert.equal(fresh[3],'Not applied','unknown peer suppresses supply dollars');

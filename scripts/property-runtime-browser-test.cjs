...
(async () => {
  ...
  try {
    ...
    assert.match(await page.locator('.pd-value-breakdown').innerText(), /19,488\.00/, 'original c006 assessment stays distinct');
    ...
    assert.match(await page.locator('.pd-value-breakdown').innerText(), /60,000\.00/, 'original r008 assessment stays distinct');
    await page.locator('#pd-detail-close').click();

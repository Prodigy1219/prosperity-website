...
(async()=>{
 ...
 try{
  for(const [name,width,height]of [['desktop',1440,1000],['mobile',390,844]]){
   ...
   await page.locator('.pd-thumb').first().click();await page.waitForFunction(()=>document.querySelector('#pd-preview-label').textContent.includes('Textured build'),{},{timeout:45000});
   assert.match(await page.locator('.pd-materials').innerText(),/57,231 occupied block cells/);
   assert.match(await page.locator('.pd-detail-info > .pd-price').innerText(),/33,640\.98/,'top estimate includes assessment + materials');
   assert.match(await page.locator('.pd-value-breakdown').innerText(),/12,096\.00[\s\S]*21,544\.98/,'original assessment stays distinct');
